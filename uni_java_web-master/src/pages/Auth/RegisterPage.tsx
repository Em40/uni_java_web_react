...
const [password, setPassword] = useState<string>('');
  ...
  return (
    <Container>
      ...
      <div className='space-y-6 w-96 m-auto text-center'>     
        <div className='mb-9 mt-24'> 
          ...
          <TextField size='small' fullWidth label='Password' type='password' value={password} onChange={(event: ChangeEvent<HTMLInputElement>)=>setPassword(event.target.value)} />
          ...
          <Button onClick={()=>loginMutate({email: email, username: username, password: password})}>Sign in</Button>
        </div>
    </Container>
  );
}
