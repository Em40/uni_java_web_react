...
import { BrandName } from '../../models/Brand';
...
const BrandCreateModal: FC<BrandCreateModalProps> = ({handleModalState, open}) => {
  ...
  const { useCreateBrand } = brandQueryClient;

  const { mutate: brandCreate } = useCreateBrand(
      ...
      >
        <Box sx={style} className='space-y-6'>
          <div>
            ...
            <Button disabled={!name} onClick={()=>brandCreate({name: name as BrandName})}>Confirm</Button>
